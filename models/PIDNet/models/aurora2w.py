...
class AuRoRA2W(nn.Module):

    def __init__(self, m=2, n=3, num_classes=19, planes=64, ppm_planes=96, head_planes=128, augment=True):
        super(AuRoRA2W, self).__init__()
    ...
    def forward(self, x, roll_angles=None):
        ...
        x_ = self.layer4_(self.relu(x_), offsets)
...
def get_seg_model(cfg, imgnet_pretrained):
    ...
    if 's' in cfg.MODEL.NAME:
        model = AuRoRA2W(m=2, n=3, num_classes=cfg.DATASET.NUM_CLASSES, planes=32, ppm_planes=96, head_planes=128, augment=True)
    elif 'm' in cfg.MODEL.NAME:
        model = AuRoRA2W(m=2, n=3, num_classes=cfg.DATASET.NUM_CLASSES, planes=64, ppm_planes=96, head_planes=128, augment=True)
    else:
        model = AuRoRA2W(m=3, n=4, num_classes=cfg.DATASET.NUM_CLASSES, planes=64, ppm_planes=112, head_planes=256, augment=True)
...
def get_pred_model(name, num_classes):
    ...
    if 's' in name:
        model = AuRoRA2W(m=2, n=3, num_classes=num_classes, planes=32, ppm_planes=96, head_planes=128, augment=False)
    elif 'm' in name:
        model = AuRoRA2W(m=2, n=3, num_classes=num_classes, planes=64, ppm_planes=96, head_planes=128, augment=False)
    else:
        model = AuRoRA2W(m=3, n=4, num_classes=num_classes, planes=64, ppm_planes=112, head_planes=256, augment=False)
